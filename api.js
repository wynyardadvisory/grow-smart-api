...
async function geocodePostcode(postcode, countryCode = "GB") {
  ...
  try {
    if (countryCode === "GB") {
      ...
      // Fallback 1: try input directly as outcode (e.g. "LE2", "GU34", "TS22", "PO12")
      // postcodes.io /outcodes/ accepts full outcodes directly
      const r2 = await fetch("https://api.postcodes.io/outcodes/" + encodeURIComponent(clean));
      const json2 = await r2.json();
      if (json2.status === 200 && json2.result?.latitude) {
        return { latitude: json2.result.latitude, longitude: json2.result.longitude };
      }
      // Fallback 2: strip trailing digits and try area code (e.g. "LE2" -> "LE")
      // Some single-letter areas like "S" or "M" may need this
      const district = clean.replace(/[0-9]+$/, "");
      if (district && district !== clean) {
...
const allowedOrigins = [
  ...
  "http://localhost",        // Android Capacitor WebView (older)
...
app.post("/cron/daily", async (req, res) => {
  ...
  try {
    const { data: profiles } = await supabaseService.from("profiles").select("id").eq("is_demo", false).limit(50000);
    if (!profiles?.length) {
      console.log("[CronDaily] No profiles found — nothing to process.");
      return res.json({ ok: true, status: "done", tasks: 0, users: 0 });
    }
    let total = 0;
    for (const p of profiles) {
      try {
    ...
    res.json({ ok: true, status: "done", tasks: total, users: profiles.length });
  } catch (err) {
